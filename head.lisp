...
(defun add-head (screen head)
  (dformat 3 "Adding head #~D" (head-number head))
...
(defun remove-head (screen head)
  (dformat 3 "Removing head #~D" (head-number head))
...
(defun replace-head (screen old-head new-head)
  ...
  (dformat 3 "Replacing head ~A with head ~A" old-head new-head)

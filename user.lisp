...
submitting the bug report."
  (if *last-unhandled-error*
      (progn
        ...
        (wm-message "Copied to clipboard."))
      (wm-message "There was no unhandled error!")))
     ...
     (loop for i in (xlib:list-properties win)
           ...
           collect (multiple-value-bind (values type)
                     ...
                     (case type
                       ...
                       (:string (format nil "~{~s~^, ~}"
                                        (mapcar (lambda (x) (map 'string 'xlib:char-from-card8 x))
                                                (split-seq values '(0)))))
